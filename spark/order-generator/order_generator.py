...
class OrderGenerator:
    ...
    def start_generating(self):
        ...
        print(f"Starting order generation at {datetime.now()}")
        ...
        try:
            while True:
                ...
                if order_count % (self.orders_per_second * 30) == 0:
                    ...
                    print(f"Generated {order_count} orders in {total_elapsed:.1f}s "
                          f"(actual rate: {actual_rate:.1f}/s)")

        except KeyboardInterrupt:
            print(f"\nStopping order generation. Total orders generated: {order_count}")
        except Exception as e:
